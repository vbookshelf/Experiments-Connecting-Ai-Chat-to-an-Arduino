...
# License: MIT
# Version: app114 variant (With function calling and unique bbox naming)
#----------------------
...
UPLOADS_FOLDER = "user_uploads" # For temporary user file uploads
...
# --- NEW --- Flag to control image feedback loop to the LLM
# If True, images created by tools are sent back to the LLM for context.
# This increases context awareness but also increases inference time.
# If False, the LLM is not shown the image, which improves speed.
ATTACH_TOOL_IMAGES = True
...
# -----------------------------------------
# START - Tools and Tool Definitions
# -----------------------------------------

# Define the serial port for the Arduino
...
def get_current_time():
    ...
    current_data_and_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
get_current_time_tool_def = {
  ...
  "function": {
    ...
    "description": "Returns the current system time in the format 'YYYY-MM-DD HH:MM:SS'.",
    "parameters": {
      "type": "object",
      "properties": {},
      "required": []
...
def create_rainbow_color_image(color):
    """
    Creates a square image of the specified rainbow color and saves it.
    ...
    if color == "red":
        ...
        raise ValueError("Invalid rainbow color.")

    image = Image.new("RGB", (100, 100), color=rgb)
    draw = ImageDraw.Draw(image)

    filename = f"{color.lower()}_rainbow_square.png"
    image.save(os.path.join(UPLOADS_FOLDER, filename))
    del draw, image

    return json.dumps({"image_generated": filename})

create_rainbow_color_image_tool_def = {
    "type": "function",
    "function": {
        "name": "create_rainbow_color_image",
        "description": "Creates a square image of the specified rainbow color and saves it.",
        "parameters": {
            "type": "object",
            "properties": {
                "color": {
                    "type": "string",
                    "description": "The rainbow color to use for the image.",
...
def show_image(animal_type: str):
    """
    Shows an image of a specific type of animal.
    Returns the file name of the image to be displayed.
    """
    if animal_type == "dog":
        ...
        return json.dumps({"error": f"Sorry, I don't have an image for '{animal_type}'."})
...
show_image_tool_def = {
    ...
    "function": {
        "name": "show_image",
        "description": "Displays a picture of an animal when the user asks to see one. Use this function to show an image of a dog or a cat.",
        "parameters": {
            "type": "object",
            "properties": {
                "animal_type": {
                    "type": "string",
                    "description": "The type of animal to show an image of.",
                    "enum": ["dog", "cat"]
                }
            },
...
def draw_bounding_box(image_filename: str, relative_bbox: list):
    """
    Draws a bounding box on an image and saves it as a new file.
    Returns the filename of the annotated image.
    """
    if not (isinstance(relative_bbox, list) and len(relative_bbox) == 4 and all(isinstance(i, (int, float)) for i in relative_bbox)):
        return json.dumps({"error": "Invalid bounding box coordinates. Must be a list of four numbers."})

    safe_filename = secure_filename(image_filename)

    path_in_uploads = os.path.join(UPLOADS_FOLDER, safe_filename)
    path_in_root = safe_filename

    source_path = None
    if os.path.exists(path_in_uploads):
        source_path = path_in_uploads
    elif os.path.exists(path_in_root):
        source_path = path_in_root
    else:
        return json.dumps({"error": f"Image file '{safe_filename}' not found."})
    ...
    try:
        with Image.open(source_path) as img:
            ...
            is_absolute = any(coord > 1.0 for coord in relative_bbox)

            if is_absolute:
                print("[INFO] Absolute pixel coordinates detected. Converting to relative.")
                ...
                x1_rel, y1_rel, x2_rel, y2_rel = relative_bbox

            final_x1 = x1_rel * img_width
            final_y1 = y1_rel * img_height
            final_x2 = x2_rel * img_width
            final_y2 = y2_rel * img_height

            draw.rectangle([final_x1, final_y1, final_x2, final_y2], outline="red", width=3)

            unique_suffix = int(time.time())
            name, ext = os.path.splitext(safe_filename)
            new_filename = f"{name}_annotated_{unique_suffix}{ext}"

            output_path = os.path.join(UPLOADS_FOLDER, new_filename)
            img.save(output_path)

            print(f"[TOOL] Bounding box drawn on '{safe_filename}', saved as '{new_filename}'.")

            return json.dumps({"image_generated": new_filename})

    except Exception as e:
        print(f"[ERROR] Failed to draw bounding box: {e}", file=sys.stderr)
        return json.dumps({"error": f"An error occurred while processing the image: {e}"})

draw_bounding_box_tool_def = {
    "type": "function",
    "function": {
        "name": "draw_bounding_box",
        "description": "When the user asks to find, locate, or highlight something in an image, use this tool to draw a box around the object. You must provide the image filename and the bounding box coordinates.",
        "parameters": {
            "type": "object",
            "properties": {
                "image_filename": {
                    "type": "string",
                    "description": "The filename of the image to draw on. This must be a file the user has uploaded or a file you have previously shown."
                },
                "relative_bbox": {
                    "type": "array",
                    "description": "A list of four floats [x1, y1, x2, y2] representing the top-left and bottom-right coordinates of the box, as values from 0.0 to 1.0.",
                    "items": {
                        "type": "number"
                    }
                }
            },
            "required": ["image_filename", "relative_bbox"]
        }
    }
}
...
def control_led(state: str):
    """
    Sends a command to an Arduino to turn the built-in LED on or off.
    Returns a JSON string confirming the action.
    """
    if state.lower() not in ['on', 'off']:
        return json.dumps({"error": "Invalid state specified. Must be 'on' or 'off'."})

    # Command string that the Arduino sketch will listen for
    command = f"LED_{state.upper()}\n"

    try:
        # Establish serial connection with a timeout
        ser = serial.Serial(PORT, 9600, timeout=2)
        # Give the connection a moment to settle, crucial for some Arduino boards
        time.sleep(2)

        # Send the command, encoded as bytes
        ser.write(command.encode('utf-8'))
        print(f"[TOOL] Sent command '{command.strip()}' to Arduino on port {PORT}.")

        # Read the confirmation response from the Arduino
        response = ser.readline().decode('utf-8').strip()
        print(f"[TOOL] Received response: '{response}'")
        ...
        if response:
            ...
            return json.dumps({"status": "error", "response": "No confirmation received from the device."})

    except serial.SerialException as e:
        print(f"[ERROR] Serial communication error: {e}", file=sys.stderr)
        return json.dumps({"error": f"Could not connect to the device on port {PORT}. Please check the connection and ensure the port is correct."})
    except Exception as e:
        print(f"[ERROR] An unexpected error occurred in control_led: {e}", file=sys.stderr)
        return json.dumps({"error": f"An unexpected error occurred: {str(e)}"})

control_led_tool_def = {
    "type": "function",
    "function": {
        "name": "control_led",
        "description": "Use this tool to switch a physical light on or off when the user asks to. For example, 'switch on the light' or 'turn the LED off'. This tool sends an instruction to an Arduino micro-controller which executes the action.",
        "parameters": {
            "type": "object",
            "properties": {
                "state": {
...
### --- ADDED SENSOR TOOL START --- ###
def get_sensor_readings():
    """
    Sends a command to an Arduino to read temperature and humidity sensors.
    Returns a JSON string with the sensor data or an error message.
    """
    ...
    try:
        ...
        if not response:
            return json.dumps({"error": "No data received from the device."})

        # Parse the JSON response from the Arduino.
        # This function returns the raw JSON string directly to the LLM.
        try:
            # Validate that it's valid JSON before returning
            json.loads(response) 
            return response
        except json.JSONDecodeError:
            print(f"[ERROR] Invalid JSON received from Arduino: {response}", file=sys.stderr)
            return json.dumps({"error": f"Invalid data format received from device: {response}"})

    except serial.SerialException as e:
        print(f"[ERROR] Serial communication error: {e}", file=sys.stderr)
        return json.dumps({"error": f"Could not connect to the device on port {PORT}. Please check the connection."})
    except Exception as e:
        print(f"[ERROR] An unexpected error occurred in get_sensor_readings: {e}", file=sys.stderr)
        return json.dumps({"error": f"An unexpected error occurred: {str(e)}"})

get_sensor_readings_tool_def = {
    "type": "function",
    "function": {
        "name": "get_sensor_readings",
        "description": "Use this tool to get the current temperature and humidity from the connected sensor device. It returns the temperature in Celsius and humidity as a percentage. This tool sends a request to an Arduino micro-controller which has a DHT22 AM2302 sensor attached. The temperature and humidity readings from this sensor are returned.",
        "parameters": {
            "type": "object",
            "properties": {},
            "required": []
...
function_dict = {
    ...
    "create_rainbow_color_image": create_rainbow_color_image,
...
# -----------------------------------------
# END - Tools and Tool Definitions
# -----------------------------------------
...
<body class="flex h-screen overflow-hidden text-slate-800">
    ...
    <script type="module">
		...
		function renderMessage(agentId, msg) {
		    ...
		    if (rawText.trim().length > 0 || !isUser) {
		        ...
		        if (isUser) {
                    ...
                    processAndRenderAiResponse(rawText, markdownDiv);
                ...
                if (agentId) {
                    ...
                    if (index !== -1) {
                        ...
                        if (activeChats[agentId]) {
                            activeChats[agentId].agent = updatedAgent; 
        ...
        function processAndRenderAiResponse(messageText, targetElement) {
            if (!targetElement) return;

            const imageTagRegex = /<show_image>(.*?)<\/show_image>/g;
            
            const processedHtml = messageText.replace(imageTagRegex, (match, filename) => {
                const trimmedFilename = filename.trim();
                if (trimmedFilename) {
                    const imageUrl = `/files/${encodeURIComponent(trimmedFilename)}`;
                    return `<br><img src="${imageUrl}" alt="${trimmedFilename.replace(/"/g, '&quot;')}" class="w-full max-w-md h-auto rounded-lg my-2 border-2 border-slate-200 shadow-sm"><br>`;
                }
                return '';
            });

            targetElement.innerHTML = marked.parse(processedHtml);
        ...
        function setupSocketListeners() {
            socket.on('llm_token', (data) => {
                ...
                if (currentAiMessageElement) {
                    ...
                    if (chat && chat.history.length > 0) {
                        ...
                        processAndRenderAiResponse(lastMsg.parts[0].text, currentAiMessageElement);
                    }
            ...
            socket.on('chat_end', async (data) => {
                ...
                if (chat && chat.history.length > 0) {
                    ...
                    if(currentAiMessageElement) {
                        processAndRenderAiResponse(data.final_message, currentAiMessageElement);
                    }
...
def format_manifest_for_prompt(manifest):
    """Formats the photo list for the system prompt."""
    if not manifest:
        return ""
    
    formatted_list = "\n".join([f"- `{item.split(':')[0]}`: {item.split(':', 1)[1].strip()}" for item in manifest])
    
    return f"The following built-in images are available:\n{formatted_list}"

image_manifest_string = format_manifest_for_prompt(photo_manifest)
...
DEFAULT_AGENT = {
    ...
    "persona": f"""You are a friendly and helpful assistant.

**Image Display Workflow:**
When you need to show an image to the user, you MUST include the following tag in your response: `<show_image>filename.jpg</show_image>`.

**Visual Analysis & Verification Workflow:**
When the user asks you to find, locate, or highlight an object within an image:
1.  **Identify:** Analyze the image and determine the bounding box coordinates of the requested object. These coordinates MUST be **relative floats between 0.0 and 1.0** (e.g., `[0.25, 0.3, 0.75, 0.8]`).
2.  **Tool Call:** Call the `draw_bounding_box` tool with the `image_filename` and the `relative_bbox` coordinates.


Do not use emojis. Use LaTeX notation for mathematical or scientific expressions only.""",
    "color": "#4f46e5",
    "type": "multi-turn",
    "isDefault": True,
    "avatar_url": None
...
def load_agents():
    ...
    try:
        with open(AGENTS_FILE, "r") as f:
            agents = json.load(f)
            # Ensure the default agent always has the latest persona from the script
            for agent in agents:
                if agent.get("isDefault"):
                    agent["persona"] = DEFAULT_AGENT["persona"]
            return agents
    except (json.JSONDecodeError, IOError):
...
def update_agent(agent_id):
    ...
    for i, agent in enumerate(all_agents):
        if agent["id"] == agent_id:
            if agent.get("isDefault"):
                if 'persona' in updated_data and updated_data['persona'] != DEFAULT_AGENT['persona']:
...
def serve_llm_image_from_root(filename):
    safe_filename = secure_filename(filename)
    if safe_filename != filename:
        return "Not Found", 404

    user_upload_path = os.path.join(UPLOADS_FOLDER, safe_filename)
    if os.path.exists(user_upload_path):
        return send_from_directory(UPLOADS_FOLDER, safe_filename)

    root_path = os.path.join('.', safe_filename)
...
def upload_file():
    ...
    if allowed_file(file.filename):
        ...
        try:
            ...
            print(f"[ERROR] Could not save uploaded file '{filename}': {e}", file=sys.stderr)
            return jsonify({'error': f"Could not save file '{filename}'."}), 500
        
        unique_id = f"{session.get('sid', 'unknown_session')}_{uuid.uuid4()}"
        
        if extension == 'pdf':
            base64_data, error = process_pdf_to_base64(file_stream, filename)
        else:
            base64_data, error = process_image_to_base64(file_stream)
        ...
        session.setdefault('uploaded_filenames', []).append(filename)
...
def process_sentence_for_tts(sentence, tts_settings):
    ...
    cleaned_sentence = re.sub(r'<show_image>.*?</show_image>', '', sentence)
    ...
    try:
        ...
        samples, sample_rate = kokoro.create(text=cleaned_sentence, voice=tts_voice, speed=tts_speed, lang=kokoro_lang)
        ...
        print(f"[ERROR] TTS generation failed for sentence '{cleaned_sentence}': {e}", file=sys.stderr)
...
def handle_disconnect():
    ...
    uploaded_filenames = session.pop('uploaded_filenames', [])
    for filename in uploaded_filenames:
        try:
            filepath = os.path.join(UPLOADS_FOLDER, filename)
            if os.path.exists(filepath):
                os.remove(filepath)
                print(f"[CLEANUP] Removed uploaded file on disconnect: {filename}")
        except OSError as e:
            print(f"[ERROR] Could not remove file {filename}: {e}", file=sys.stderr)
...
def handle_chat_message(data):
    ...
    for msg in history:
        ...
        image_filenames = part.get("image_filenames", [])

        if msg["role"] == "user" and image_filenames:
            image_metadata = {"file_paths": image_filenames}
            content_payload = {
        ...
        if content:
            filenames = re.findall(r'<show_image>(.*?)</show_image>', content)
            for filename in filenames:
                safe_filename = secure_filename(filename.strip())
                path_to_check_uploads = os.path.join(UPLOADS_FOLDER, safe_filename)
                path_to_check_root = safe_filename
                
                image_path = None
                if os.path.exists(path_to_check_uploads):
                    image_path = path_to_check_uploads
                elif os.path.exists(path_to_check_root):
                    image_path = path_to_check_root

                if image_path:
                    try:
                        with open(image_path, "rb") as image_file:
                            b64_string = base64.b64encode(image_file.read()).decode('utf-8')
                            images_base64_list.append(b64_string)
                            print(f"[INFO] Re-hydrated image '{safe_filename}' from history for LLM context.")
                    except Exception as e:
                        print(f"[ERROR] Failed to read or encode image '{safe_filename}' from history: {e}")
                else:
                    print(f"[WARNING] Image '{safe_filename}' from history not found on disk.")
    ...
    try:
        ...
        if initial_response['message'].get('tool_calls'):
            print("[INFO] Tool call detected. Executing functions...")
            ...
            for tool_call in initial_response['message']['tool_calls']:
                ...
                tool_result_content = ""
                images_for_this_tool_call = []

                if function_name in function_dict:
                    print(f"  - Executing: {function_name} with args {function_args}")
                    result = function_dict[function_name](**function_args) if function_args else function_dict[function_name]()
                    
                    try:
                        result_data = json.loads(result)
                        if isinstance(result_data, dict) and "image_generated" in result_data:
                            filename = result_data["image_generated"]
                            tool_response_payload = {
                                "tool_output": f"Successfully generated image: {filename}",
                                "image_metadata": {"file_paths": [filename]}
                            }
                            tool_result_content = json.dumps(tool_response_payload)
                            print(f"[INFO] Function generated an image: {filename}. Preparing to feed it back to LLM.")
                            
                            path_to_check_uploads = os.path.join(UPLOADS_FOLDER, filename)
                            path_to_check_root = filename
                            ...
                            if image_path:
                                with open(image_path, "rb") as image_file:
                                    b64_string = base64.b64encode(image_file.read()).decode('utf-8')
                                    if ATTACH_TOOL_IMAGES:
                                        images_for_this_tool_call.append(b64_string)
                            else:
                                print(f"[ERROR] Generated image file '{filename}' not found on disk.")
                        else:
                            tool_result_content = str(result)
                    except (json.JSONDecodeError, TypeError):
                        tool_result_content = str(result)
                    
                    tool_message = {"role": "tool", "content": tool_result_content}
                    if images_for_this_tool_call:
                        tool_message["images"] = images_for_this_tool_call
                    messages.append(tool_message)

                else:
                    print(f"[ERROR] Function '{function_name}' not found.")
                    messages.append({"role": "tool", "content": f"Error: Function '{function_name}' not found."})
        ...
        if response_stream:
            ...
            for chunk in response_stream:
                ...
                if chunk.get("done"):
                    print("\n[INFO] Finished streaming response after tool call.")
                    ...
                    if total_tokens >= (current_num_ctx * 0.9):
                        warning_msg = f"Context Warning: The chat has used {total_tokens} tokens, which is over 90% of the {current_num_ctx} token limit."
            ...
            print("[INFO] No tool call. Simulating stream from initial response.")
            full_response = initial_response['message']['content'] or ""
            sentence_buffer = ""
            ...
            for token in full_response:
                ...
                if tts_enabled == "On":
                    ...
                    complete_sentences = split_into_sentences(sentence_buffer)
                    if len(complete_sentences) > 1:
                        for s in complete_sentences[:-1]: process_sentence_for_tts(s, tts_settings)
                        sentence_buffer = complete_sentences[-1]
                    elif len(complete_sentences) == 1 and sentence_buffer.strip().endswith(('.', '!', '?')):
                        process_sentence_for_tts(complete_sentences[0], tts_settings)
                        sentence_buffer = ""
            ...
            print("\n[INFO] Finished processing non-tool response.")
            prompt_tokens = initial_response.get('prompt_eval_count', 0) or 0
            completion_tokens = initial_response.get('eval_count', 0) or 0
            eval_duration_ns = initial_response.get('eval_duration', 0) or 0
